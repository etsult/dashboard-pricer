...
def load_ohlcv_synthetic(
    ...
    """Public alias for synthetic OHLCV generation with optional vol override."""
    return _synthetic_ohlcv(symbol, n_bars=n_bars, seed=seed, sigma_daily=sigma_daily)


def _synthetic_ohlcv(symbol: str, n_bars: int = 1440, seed: int = 42,
                     sigma_daily: float | None = None) -> pd.DataFrame:
    """
    ...
    if sigma_daily is not None:
        ann_vol = sigma_daily * np.sqrt(365)
    ...
    # Bar half-range ~ [0.5 * sigma_dt, 2 * sigma_dt] (matches GBM statistics)
    noise  = rng.uniform(0.5 * sigma_dt, 2.0 * sigma_dt, n_bars)
