...
class BacktestResult:
    ...
    def compute_metrics(self) -> dict:
        ...
        if len(pnl) > 1:
            daily_pnl_series = _to_daily(self.timestamps, pnl)
            if len(daily_pnl_series) > 1:
                mean_d = sum(daily_pnl_series) / len(daily_pnl_series)
                std_d  = math.sqrt(sum((x - mean_d)**2 for x in daily_pnl_series) / max(len(daily_pnl_series)-1, 1))
        ...
        fill_rate_pct = n_fills / max(len(self.timestamps), 1) * 100
        ...
        self.metrics = {
            ...
            # Spread
            "avg_quoted_spread_bps": round(avg_quoted_half * 2, 2),
...
def _to_daily(timestamps: Sequence[float], values: Sequence[float]) -> list[float]:
    """Aggregate a running P&L series into daily returns (end-of-day minus start-of-day)."""
    if not timestamps:
        return []
    # Record the first value seen for each calendar day
